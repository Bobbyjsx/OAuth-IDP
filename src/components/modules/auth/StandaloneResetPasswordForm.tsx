...
import { getServerError, useStandaloneResetPassword } from "@/api";
...
export function StandaloneResetPasswordForm() {
  ...
  const { mutate: performReset, isPending } = useStandaloneResetPassword<{
    reset_token: string;
    new_password: string;
      ...
      <div className="w-full relative">
        ...
        >
          <p className="text-body-md text-gray-medium dark:text-zinc-400 font-medium leading-relaxed">
            Your password has been successfully reset. You can now close this tab and sign in to
            your application.
          </p>
        </motion.div>
      </div>
